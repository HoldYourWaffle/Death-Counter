...
public class CommandDeathCounter extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender commandSender, String[] args) throws CommandException {
		...
		if ("leaderboard".toLowerCase().startsWith(args[0])) {
			...
			if (args[1].equalsIgnoreCase("all")) {
				...
				if (DeathCounter.handler.clearDeath(args[1])) {
	...
	public static void broadcastLeaderboard(String s) {
		...
		if (s != null) {
			if (!s.equals("Server")) {
				if (FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayerByUsername(s) == null) return;
		...
		if (s != null && s.equals("Server")) {
			if (DeathHandler.ranking.isEmpty()) DeathCounter.console("No deaths have occured on this server yet.", Level.INFO);
			else {
				DeathCounter.console("Death Leaderboard:", Level.INFO);
				for (int i = 1; i <= DeathHandler.leaderboardCount && i - 1 < DeathHandler.ranking.size(); i++) {
					DeathCounter.console("   " + DeathCounter.handler.getDisplayedRank(DeathHandler.ranking.get(i - 1)) + "  " + DeathHandler.ranking.get(i - 1) + " (" + DeathCounter.handler.getDeathCount(DeathHandler.ranking.get(i - 1)) + (DeathCounter.handler.getDeathCount(DeathHandler.ranking.get(i - 1)) == 1 ? " Death)" : " Deaths)"), Level.INFO);
				}
			}
		} else {
			for (EntityPlayer player : playersToNotify) {
				if (DeathHandler.ranking.isEmpty()) player.sendStatusMessage(new TextComponentTranslation("dc.command.noDeaths"), false);
				else {
					boolean isPlayer = false;
					player.sendStatusMessage(new TextComponentTranslation("dc.command.leaderboard"), false);
					for (int i = 1; i <= DeathHandler.leaderboardCount && i - 1 < DeathHandler.ranking.size(); i++) {
						String playerName = DeathHandler.ranking.get(i - 1);
						if (playerName.equals(player.getName())) isPlayer = true;

						int rank = DeathCounter.handler.getDisplayedRank(playerName);
						player.sendStatusMessage(new TextComponentString((rank == 1 ? "\u00A7e" : rank == 2 ? "\u00A77" : rank == 3 ? "\u00A74" : "") + (playerName.equals(player.getName()) ? " ->" : "    ") + rank + "  " + DeathHandler.ranking.get(i - 1) + " (" + DeathCounter.handler.getDeathCount(DeathHandler.ranking.get(i - 1)) + (DeathCounter.handler.getDeathCount(DeathHandler.ranking.get(i - 1)) == 1 ? " Death)" : " Deaths)")), false);
					}
					
					if (!isPlayer) {
						int rank = DeathCounter.handler.getDisplayedRank(player.getName());
						player.sendStatusMessage(new TextComponentString(" ->" + rank + (rank >= 10 ? " " : "  ") + player.getName() + " (" + DeathCounter.handler.getDeathCount(player.getName()) + (DeathCounter.handler.getDeathCount(player.getName()) == 1 ? " Death)" : " Deaths)")), false);
					}
				}
			}
		}
