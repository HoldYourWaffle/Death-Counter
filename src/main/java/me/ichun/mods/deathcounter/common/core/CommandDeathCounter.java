...
public class CommandDeathCounter extends CommandBase {
	...
	public static void broadcastLeaderboard(String s) {
		...
		if (s != null && s.equals("Server")) {
			if (DeathCounter.ranking.isEmpty()) DeathCounter.console("No deaths have occured on this server yet.", Level.INFO);
			else {
			...
			for (EntityPlayer player : playersToNotify) {
				if (DeathCounter.ranking.isEmpty()) player.sendStatusMessage(new TextComponentTranslation("dc.command.noDeaths"), false);
				else {
